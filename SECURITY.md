...
agent-sync uses a strict file allowlist. It does not sync login credentials, `auth.json`, `.credentials.json`, SQLite database files, caches, settings, or browser state. For Codex paginated history, only four known conversation tables are exported as per-thread JSON. Imports require matching session identity and log checksums, validate the locally installed schema, and preserve other threads. Database schema or SQL statements supplied by a remote are never executed. This is not message-content redaction: a key pasted into a conversation remains in that conversation.
